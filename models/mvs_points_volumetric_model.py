...
class MvsPointsVolumetricModel(NeuralPointsVolumetricModel):
    ...
    def setup_optimizer(self, opt):
        ...
        #     total=0
        #     for block in model.named_parameters():
        #         if block[1].requires_grad:
        #             if not block[0].startswith("module.neural_points"):
        #                 block[1].numel()
        #                 total = total + block[1].numel()
        #                 print(block[1].numel())
        #     return total
    ...
    def forward(self):
        ...
        if not self.opt.no_loss:
            if self.opt.is_train and (self.blur_kernels is not None) and (self.output["blur_predictor"] is None) and self.opt.add_blur_sim:
                self.blur_update_output()
            elif self.opt.is_train and (self.output["blur_predictor"] is not None) and self.opt.add_blur_sim:
                self.learnable_blur_update_output(self.output["blur_predictor"])
            else:
                pass
                # print('No blur simulation is applied')
            self.compute_losses(color_patch_loss=False, color_patch_match_region=0, combine_two=False, add_ssim_loss=False, detail_aware=False)
