...
def ray_march(ray_dist,
    ...
    if bg_color is not None:
        if ray_color.shape[-1] == 3:
            ray_color += bg_color.to(opacity.device).float().view(
                background_transmission.shape[0], 1, 3) * background_transmission
        else:
            ray_color += bg_color.tile(1, int(ray_color.shape[-1]/3)).to(opacity.device).float().view(
                background_transmission.shape[0], 1, ray_color.shape[-1]) * background_transmission
