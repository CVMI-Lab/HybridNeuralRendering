...
class NeuralPointsVolumetricModel(BaseRenderingModel):
    ...
    def fill_invalid(self, output, input):
        ...
        if "bg_ray" in self.input:
            ...
            coarse_raycolor_tensor = self.tonemap_func(torch.ones([B, OR, 3], dtype=output["coarse_raycolor"].dtype, device=output["coarse_raycolor"].device) * input["bg_color"][None, ...])
            coarse_raycolor_patch_tensor = self.tonemap_func(torch.ones([B, OR, 3], dtype=output["coarse_raycolor"].dtype, device=output["coarse_raycolor"].device) * input["bg_color"][None, ...]).tile(1, 1, int(output["coarse_raycolor_patch"].shape[-1]/3))
            coarse_raycolor_tensor[ray_inds[..., 0], ray_inds[..., 1], :] = output["coarse_raycolor"]
            coarse_raycolor_patch_tensor[ray_inds[..., 0], ray_inds[..., 1], :] = output["coarse_raycolor_patch"]

        output["coarse_raycolor"] = coarse_raycolor_tensor
        output["coarse_raycolor_patch"] = coarse_raycolor_patch_tensor
    ...
    def create_network_models(self, opt):
        ...
        # network
        self.net_ray_marching = NeuralPointsRayMarching(**params, **self.found_funcs)
        ...
        if self.opt.gpu_ids:
            ...
            self.net_ray_marching = torch.nn.DataParallel(self.net_ray_marching, self.opt.gpu_ids)
...
class NeuralPointsRayMarching(nn.Module):
    ...
    def w2iproject(self, sample_loc_w, intrinsic, c2w, pixel_idx):
        sample_loc_w_h = torch.cat([sample_loc_w, torch.ones_like(sample_loc_w[..., :1])], dim=-1)
        w2c = torch.inverse(c2w).t()
        sample_loc_c = sample_loc_w_h @ w2c
        sample_loc_i = sample_loc_c[:, :, :3] @ intrinsic.t()
        sample_depth = sample_loc_i[:, :, 2:3]
        sample_loc_i = sample_loc_i / (sample_depth + 1e-10)
        return sample_loc_i[:, :, 0:2]
    ...
    def forward(self,
                ...
                images_nearest=None,
                campos_nearest=None,
                intrinsic_nearest=None,
                vid_angle_nearest=None,
                frame_weight_nearest=None,
                **kargs):
        output = {}
        ...
        # warp the sampled points onto the nearest xxx image plane
        if self.opt.dynamic_nearest:
            self.opt.use_nearest = c2w_nearest.shape[1]

        sample_loc_i_nearest = []
        for nearest_idx in range(self.opt.use_nearest):
            sample_loc_i_nearest_tmp = self.w2iproject(sample_loc_w[0], intrinsic_nearest[0], c2w=c2w_nearest[0, nearest_idx], pixel_idx=pixel_idx)
            sample_loc_i_nearest.append(sample_loc_i_nearest_tmp)
        if self.opt.use_nearest == 0:
            sample_loc_i_nearest = torch.zeros_like(sample_loc_w)[:, :, :, 0:2]
        else:
            sample_loc_i_nearest = torch.stack(sample_loc_i_nearest)

        # calculate delta view directions between nearest and current views.
        delta_sample_viewdir_nearest = []
        sample_viewdir_current = sample_loc_w - campos[0]
        sample_viewdir_current_norm = torch.linalg.norm(sample_viewdir_current, dim=-1, keepdim=True)
        sample_viewdir_current = sample_viewdir_current / (sample_viewdir_current_norm + 1e-6)
        for nearest_idx in range(self.opt.use_nearest):
            sample_viewdir_nearest_tmp = sample_loc_w - campos_nearest[0, nearest_idx, :]
            sample_viewdir_nearest_tmp_norm = torch.linalg.norm(sample_viewdir_nearest_tmp, dim=-1, keepdim=True)
            sample_viewdir_nearest_tmp = sample_viewdir_nearest_tmp / (sample_viewdir_nearest_tmp_norm + 1e-6)
            delta_sample_viewdir_nearest_tmp = sample_viewdir_nearest_tmp - sample_viewdir_current  # the delta view direction
            delta_sample_viewdir_nearest.append(delta_sample_viewdir_nearest_tmp[0])
        if self.opt.use_nearest == 0:
            delta_sample_viewdir_nearest = torch.zeros_like(sample_loc_w)[:, :, :, 0:3]
        else:
            delta_sample_viewdir_nearest = torch.stack(delta_sample_viewdir_nearest)

        if self.opt.is_train:
            decoded_features, ray_valid, weight, conf_coefficient, blur_predictor = self.aggregator(sampled_color, sampled_Rw2c,
                                                                                    ...
                                                                                    aux_image=aux_image,
                                                                                    pixel_idx=pixel_idx,
                                                                                    img_n=images_nearest,
                                                                                    vid_angle_n=vid_angle_nearest,
                                                                                    sample_loc_i_n=sample_loc_i_nearest,
                                                                                    delta_viewdir_n=delta_sample_viewdir_nearest,
                                                                                    frame_weight_n=frame_weight_nearest)
        else:
            blur_predictor = None
            decoded_features, ray_valid, weight, conf_coefficient = self.aggregator(sampled_color, sampled_Rw2c, sampled_dir, sampled_conf, sampled_embedding, sampled_xyz_pers, sampled_xyz, sample_pnt_mask, sample_loc, sample_loc_w, sample_ray_dirs, vsize, grid_vox_sz, aux_image=aux_image, pixel_idx=pixel_idx, img_n=images_nearest, vid_angle_n=vid_angle_nearest, sample_loc_i_n=sample_loc_i_nearest, delta_viewdir_n=delta_sample_viewdir_nearest, frame_weight_n=frame_weight_nearest)
        output["blur_predictor"] = blur_predictor
        ...
        if self.return_color:
            ...
            output["coarse_raycolor_patch"] = ray_color  # each query point will output a patch, but not used currently.
            center_idx = int(ray_color.shape[-1]/2)
            ray_color_center = ray_color[:, :, (center_idx-1):(center_idx+2)]
            output["coarse_raycolor"] = ray_color_center
